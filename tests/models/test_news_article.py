...
def article():
    return NewsArticle(
        title="Test",
        body="Content",  # Changed from content to body to match your class
        url="http://test.com",
        published="2024-02-15"  # Added required published field
    )

def test_article_creation(article):
    assert article.title == "Test"
    assert article.body == "Content"  # Changed from content to body
